...
// Додаємо нові змінні на початку файлу
let dragStartIndex;
let dragEndIndex;
...
function addTodoToDOM(todo) {
    ...
    li.draggable = true; // Робимо елемент перетягуваним
...
// Додаємо нові функції для drag-n-drop
function dragStart() {
    dragStartIndex = [...todoList.querySelectorAll('li')].indexOf(this);
...
function dragDrop() {
    dragEndIndex = [...todoList.querySelectorAll('li')].indexOf(this);
...
function swapItems(fromIndex, toIndex) {
    ...
    todos.splice(toIndex, 0, itemOne);
    saveToLocalStorage(todos);
    applyFilter();
}